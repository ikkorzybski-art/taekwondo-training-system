...
from .models import (UserProfile, TrainingDay, Attendance, Flashcard, 
                     Quiz, QuizQuestion, QuizAnswer, QuizAttempt, PrivateLesson, Exam)
...
class QuizQuestionAdmin(admin.ModelAdmin):
    list_display = ['quiz', 'get_question_preview', 'question_type', 'order', 'points']
...
# Private Lesson Admin
@admin.register(PrivateLesson)
class PrivateLessonAdmin(admin.ModelAdmin):
    list_display = ['date', 'start_time', 'end_time', 'instructor', 'student', 'status']
    list_filter = ['status', 'date', 'instructor']
    search_fields = ['instructor__username', 'student__username', 'notes']
    list_editable = ['status']
    readonly_fields = ['created_at', 'updated_at']
    
    fieldsets = (
        ('Informacje podstawowe', {
            'fields': ('instructor', 'date', 'start_time', 'end_time')
        }),
        ('Rezerwacja', {
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "instructor":
            # Pokaż tylko użytkowników ze statusem staff (instruktorzy)
...
class ExamAdmin(admin.ModelAdmin):
    list_display = ('exam_type', 'date', 'time', 'location', 'examiner', 'max_participants')
    ...
    fieldsets = (
        ('Podstawowe informacje', {
            'fields': ('exam_type', 'date', 'time', 'location')
        }),
        ('Szczegóły', {
            'fields': ('examiner', 'max_participants', 'description')
        }),
        ('Daty systemowe', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
